...
for i in range(len(ping_set[0])):
    temp = ping_set[0][i] == ping_set[1][i]
    f.append(temp)
print(f)
for i in range(len(f)):
    if f[i] == True:
        if f[i+1] == True and f[i-1] == False:
            for j in range(len(f)-i-1):
                if f[i+1+j] == False:
                    print("故障サブネット(" + IPv4_type[0] + ', ' + IPv4_type[1] + ")")
                    print("故障期間" + Date_set[0][i][:-2] + "〜" + Date_set[0][i+1+j][:-2])
                    break

                elif N==1 and f[i+1] != True and f[i-1] != True:
                    print("故障サブネット(" + IPv4_type[0] + ', ' + IPv4_type[1] + ")")
                    print("故障期間" + Date_set[0][i][:-2] + "〜" + Date_set[0][i+1][:-2])
